...
class TimeoutCalculator:
    ...
    def __compute_new_mean_estimate(old_mean: float, latest_rtt: float, alpha: float) -> float:
        ...
        # According from the equation of EWMA in the slides: μ ← (1 − α).μ + α.RTT
        new_mean = alpha * latest_rtt + (1 - alpha) * old_mean
        return new_mean
    ...
    def __compute_timeout(mean: float, stddiv: float, k: float, bounds: TimeoutBounds) -> float:
        ...
        # Based on the equation from slides: timeout ← μ + k.σ
        timeout = mean + k * stddiv
        if bounds.min or bounds.max:
            timeout = max(bounds.min, timeout)
            timeout = min(bounds.max, timeout)
        return timeout
